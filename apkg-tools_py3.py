...
class Apkg:
    ...
    def create(self, folder, dest_dir=None):
        ...
        apk_file = dest_dir + '/' + app_info['general']['package'] + '_' + app_info['general']['version'] + '_' + \
                   app_info['general']['architecture'] + '.' + self.apk_format['suffix']
        ...
        # archive apk file
        with Chdir(tmp_dir):
            self.__zip_archive(apk_file, [self.apk_file_contents['version'], self.apk_file_contents['control'],
                                          self.apk_file_contents['data']])
    ...
    def extract(self, package, dest_dir=None):
        ...
        # move dir
        if apkg_version == '1.0':
            app_dir = dest_dir + '/' + apk_info['app']['name'] + '_' + apk_info['app']['version'] + '_' + \
                      apk_info['app']['architecture']
        elif apkg_version == '2.0':
            app_dir = dest_dir + '/' + apk_info['general']['name'] + '_' + apk_info['general']['version'] + '_' + \
                      apk_info['general']['architecture']
    ...
    def convert(self, package):
        ...
        try:
            app_new_info['register']['boot-priority']['start-order'] = \
            app_old_info['install']['service-reg']['priority']['start']
        except KeyError:
            pass

        try:
            app_new_info['register']['boot-priority']['stop-order'] = \
            app_old_info['install']['service-reg']['priority']['stop']
        except KeyError:
            pass
